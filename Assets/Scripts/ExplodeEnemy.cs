...
public class ExplodeEnemy : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        {
            // Explode if the player is within range
            StartCoroutine("Explode");
        }
        else if (distanceToPlayer < enemyStats.explodeDist * 2)
        {
            // Add force to make the enemy move towards the player when the player is very close
            Vector3 directionToPlayer = (playerTransform.position - transform.position).normalized;
            rb.AddForce(directionToPlayer * enemyStats.jumpVelocity * 0.5f);
        }
    }
